...
func setup_player_squad():
	...
	var unit_datas :Array = NetworkLobbyManager.player_extra_data["units"]
	for i in unit_datas:
		var unit :InfantryData = InfantryData.new()
		unit.from_dictionary(i)
		player_squad_units[unit.player_id] = unit
	
	player_team = NetworkLobbyManager.player_extra_data["team"]
...
func on_infantry_spawned(_data :InfantryData, _infantry : Infantry):
	...
	if _data.team == player_team:
		var highlight :Highlight = preload("res://assets/utils/highlight/highlight.tscn").instance()
		highlight.unit = _infantry
		
		if player_squad_units.has(_data.player_id):
			highlight.highlight_color = Color(1, 1, 1, 0.8)
			player_units_highlight[_infantry] = highlight
			
		else:
			highlight.highlight_color = Color(0, 0, 1, 0.8)
			
		add_child(highlight)
		
		var wounded_marker :WoundedMarker = preload("res://assets/utils/wounded_marker/wounded_marker.tscn").instance()
		wounded_marker.revive_time = 10
		wounded_marker.expire_time = 35
		wounded_marker.wounded_unit = _infantry
		add_child(wounded_marker)
		
		_infantry.connect("dead", self, "_on_team_infantry_dead", [_data, wounded_marker])
		_infantry.connect("reset", self, "_on_team_infantry_reset", [_data, wounded_marker])
		
################################################################
# team unit signals handler
func _on_team_infantry_dead(_unit :BaseUnit, _from :Vector3, _by :String, _data :InfantryData, wounded_marker :WoundedMarker):
...
func _on_team_infantry_reset(_unit :BaseUnit, _data :InfantryData, wounded_marker :WoundedMarker):
	if _unit == player_squad.squad_leader:
		_unit.is_bot = false
		_ui.refresh_inventory_info(player_squad.squad_leader)
